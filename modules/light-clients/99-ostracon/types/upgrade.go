...
// in client state that must be the same across all valid Ostracon clients for the new chain.
// VerifyUpgrade will return an error if:
// - the upgradedClient is not a Ostracon ClientState
...
func (cs ClientState) VerifyUpgradeAndUpdateState(
	...
	if !ok {
		return nil, nil, sdkerrors.Wrapf(clienttypes.ErrInvalidClientType, "upgraded client must be Ostracon client. expected: %T got: %T",
			&ClientState{}, upgradedClient)
	}
	tmUpgradeConsState, ok := upgradedConsState.(*ConsensusState)
	if !ok {
		return nil, nil, sdkerrors.Wrapf(clienttypes.ErrInvalidConsensus, "upgraded consensus state must be Ostracon consensus state. expected %T, got: %T",
