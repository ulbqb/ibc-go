...
func (suite *TendermintTestSuite) TestCheckMisbehaviourAndUpdateState() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			height,
			&types.Misbehaviour{
				Header1:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), height, suite.now, bothValSet, bothValSet, bothSigners),
				Header2:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), height, suite.now.Add(time.Minute), bothValSet, bothValSet, bothSigners),
				ClientId: chainID,
			},
			suite.now,
			true,
		},
		{
			...
			height,
			&types.Misbehaviour{
				Header1:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+3), height, suite.now, bothValSet, bothValSet, bothSigners),
				Header2:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), height, suite.now, bothValSet, bothValSet, bothSigners),
				ClientId: chainID,
			},
			suite.now,
			true,
		},
		{
			...
			&types.Misbehaviour{
				Header1:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+3), height, suite.now, bothValSet, bothValSet, bothSigners),
				Header2:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), height, suite.now.Add(time.Hour), bothValSet, bothValSet, bothSigners),
				ClientId: chainID,
			},
			suite.now,
			true,
		},
		{
			...
			heightMinus1,
			&types.Misbehaviour{
				Header1:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), heightMinus1, suite.now, bothValSet, bothValSet, bothSigners),
				Header2:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), heightMinus1, suite.now.Add(time.Minute), bothValSet, bothValSet, bothSigners),
				ClientId: chainID,
			},
			suite.now,
			true,
		},
		{
			...
			&types.Misbehaviour{
				Header1:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), heightMinus1, suite.now, bothValSet, bothValSet, bothSigners),
				Header2:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), heightMinus3, suite.now.Add(time.Minute), bothValSet, suite.valSet, bothSigners),
				ClientId: chainID,
			},
			suite.now,
			true,
		},
		{
			...
			heightMinus3,
			&types.Misbehaviour{
				Header1:  suite.chainA.CreateOCClientHeader(chainIDRevision0, int64(height.RevisionHeight+1), heightMinus1, suite.now, bothValSet, bothValSet, bothSigners),
				Header2:  suite.chainA.CreateOCClientHeader(chainIDRevision0, int64(height.RevisionHeight+1), heightMinus3, suite.now.Add(time.Minute), bothValSet, suite.valSet, bothSigners),
				ClientId: chainID,
			},
			suite.now,
			true,
		},
		{
			...
			heightMinus3,
			&types.Misbehaviour{
				Header1:  suite.chainA.CreateOCClientHeader(chainIDRevision0, 3, heightMinus1, suite.now, bothValSet, bothValSet, bothSigners),
				Header2:  suite.chainA.CreateOCClientHeader(chainIDRevision0, 3, heightMinus3, suite.now.Add(time.Minute), bothValSet, suite.valSet, bothSigners),
				ClientId: chainID,
			},
			suite.now,
			true,
		},
		{
			...
			heightMinus3,
			&types.Misbehaviour{
				Header1:  suite.chainA.CreateOCClientHeader(chainIDRevision1, 1, heightMinus1, suite.now, bothValSet, bothValSet, bothSigners),
				Header2:  suite.chainA.CreateOCClientHeader(chainIDRevision1, 1, heightMinus3, suite.now.Add(time.Minute), bothValSet, suite.valSet, bothSigners),
				ClientId: chainID,
			},
			suite.now,
			true,
		},
		{
			...
			height,
			&types.Misbehaviour{
				Header1:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), height, suite.now, bothValSet, suite.valSet, bothSigners),
				Header2:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), height, suite.now.Add(time.Minute), bothValSet, suite.valSet, bothSigners),
				ClientId: chainID,
			},
			suite.now,
			true,
		...
		},
		{
			...
			height,
			&types.Misbehaviour{
				Header1:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+3), height, suite.now.Add(time.Minute), bothValSet, bothValSet, bothSigners),
				Header2:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), height, suite.now, bothValSet, bothValSet, bothSigners),
				ClientId: chainID,
			},
			suite.now,
		...
		},
		{
			...
			height,
			&types.Misbehaviour{
				Header1:  suite.chainA.CreateOCClientHeader("ethermint", int64(height.RevisionHeight+1), height, suite.now, bothValSet, bothValSet, bothSigners),
				Header2:  suite.chainA.CreateOCClientHeader("ethermint", int64(height.RevisionHeight+1), height, suite.now.Add(time.Minute), bothValSet, bothValSet, bothSigners),
				ClientId: chainID,
			},
			suite.now,
			false,
		...
		},
		{
			...
			&types.Misbehaviour{
				Header1:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), heightMinus1, suite.now, bothValSet, bothValSet, bothSigners),
				Header2:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), heightMinus3, suite.now.Add(time.Minute), bothValSet, bothValSet, bothSigners),
				ClientId: chainID,
			},
			suite.now,
			false,
		...
		},
		{
			...
			height,
			&types.Misbehaviour{
				Header1:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), height, suite.now, altValSet, bothValSet, altSigners),
				Header2:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), height, suite.now.Add(time.Minute), bothValSet, bothValSet, bothSigners),
				ClientId: chainID,
			},
			suite.now,
		...
		},
		{
			...
			&types.Misbehaviour{
				Header1:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), height, suite.now, bothValSet, bothValSet, bothSigners),
				Header2:  suite.chainA.CreateOCClientHeader(chainID, int64(height.RevisionHeight+1), height, suite.now.Add(time.Minute), altValSet, bothValSet, altSigners),
				ClientId: chainID,
			},
			suite.now,
			false,
